...
try 40 'int f(x) { return x+x; } int main() { return f(f(10)); }'
try 50 'int add(x,y) { return x+y; } int main() { return add(22, 28); }'
try 60 'int f(x) { if(x==0) return 60; else return f(x-1); } int main() { return f(5); }'
try 70 'int f(a,b,c,d,e,f) { return a+b+c+d+e+f; } int main() { return f(10,10,10,10,10,20); }'
try 6 'int fac(x) { if(x>0) return x*fac(x-1); return 1; } int main() { return fac(3); }'
try 7 'int f(x) { return x; } int main() { return f(7); }'
try 8 'int fib(x) { if(x<3) return 1; return fib(x-1) + fib(x-2); } int main() { return fib(6); }'
